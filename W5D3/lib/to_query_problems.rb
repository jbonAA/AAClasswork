...
def price_like_shiny_mouse
  ...
  WHERE
    shiny.name = 'Shiny Mouse' AND toys.name != shiny.name
  SQL
end
...
def just_like_orange
  ...
  FROM
    cats
  JOIN
    cats AS newcats ON cats.breed = newcats.breed
  WHERE
    newcats.name = 'Orange' AND cats.name != 'Orange'
  ORDER BY
    cats.name
  SQL
end
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
  SELECT
    newcats.name, toys.name
  FROM
    toys
  JOIN
    cattoys ON toys.id = cattoys.toy_id
  JOIN
    cats as cats ON cats.id = cattoys.cat_id
  JOIN
    cattoys AS extras ON toys.id = extras.toy_id
  JOIN
    cats AS newcats ON extras.cat_id = newcats.id
  WHERE
    newcats.name != 'Jet' AND cats.name = 'Jet'
  ORDER BY
    newcats.name
  SQL
end
